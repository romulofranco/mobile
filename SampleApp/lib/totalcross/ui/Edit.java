...
public class Edit extends Control implements TextControl, HasValue<String> {
  ...
  /** Used during the material caption animation */
  private boolean postPopupKCC;
  
  private FloatingLabel<Edit> materialCaption;
  ...
  protected void onBoundsChanged(boolean screenChanged) // guich
  {
    ...
    if (materialCaption != null) {
      ...
      materialCaption.ycap0 = materialCaption.ycap = chars.length() == 0 ? getTextY() : 0;
    }
  }
  ...
  protected void draw(Graphics g) {
        ...
        if (hasBorder && uiAndroid) {
          try {
            if (npback == null || focusColor != -1) {
              npback = NinePatch.getInstance().getNormalInstance(NinePatch.EDIT, width, height, isEnabled()
                  ? hasFocus && focusColor != -1 ? focusColor : back0
                  : (back0 == parent.backColor ? Color.darker(back0, 32) : Color.interpolate(back0, parent.backColor)),
                  false);
    ...
    if (len > 0 || drawCaption || captionIcon != null) {
      ...
      if (drawCaption && !uiMaterial) {
        ...
        case PASSWORD: // password fields usually have small text, so this method does not have to be very optimized
          if (len > 0) {
            g.drawText(Convert.dup('*', len - 1) + chars.charAt(len - 1), xx, y, textShadowColor != -1,
                textShadowColor);
          }
          break;
        case PASSWORD_ALL:
    ...
    if (uiMaterial) {
      ...
      g.setFont(materialCaption.fcap);
      g.drawText(caption, materialCaption.xcap + getX0(), materialCaption.ycap);
    }
  }
  ...
  public void popupKCC() {
    ...
    case KBD_CALENDAR:
      calendar = new CalendarBox(this.foreColor);
      ...
      if (captionPress != null && caption != null && pe.y <= materialCaption.getExtraHeight()) {
        ...
        if (kbdType != KBD_NONE && virtualKeyboard && !hadParentScrolled()) {
          if (!autoSelect && clipboardDelay != -1 && startSelectPos != -1 && startSelectPos != insertPos) {
            ...
            if (materialCaption != null && materialCaption.isRunning) {
              postPopupKCC = true;
            } else {
              popupKCC();
            }
