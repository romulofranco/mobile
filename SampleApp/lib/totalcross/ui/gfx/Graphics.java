...
public final class Graphics {
  ...
  public void fillRoundRect(int xx, int yy, int width, int height, int r) {
    ...
    do {
      ...
      if (i < 256 && i > 0) {
        xm = px2;
        ym = py2;
        setPixelAA(xm - x, ym + y, backColor, i); // br
        xm = px1;
        ym = py2;
        setPixelAA(xm - y, ym - x, backColor, i); // bl
        xm = px1;
        ym = py1;
        setPixelAA(xm + x, ym - y, backColor, i); // tl
        xm = px2;
        ym = py1;
        setPixelAA(xm + y, ym + x, backColor, i); // tr
      }
      e2 = err;
      x2 = x;
      if (err + y > 0) {
        ...
        if (i < 256 && i > 0) {
          xm = px2;
          ym = py2;
          setPixelAA(xm - x, ym + y + 1, backColor, i);
          xm = px1;
          ym = py2;
          setPixelAA(xm - y - 1, ym - x, backColor, i);
          xm = px1;
          ym = py1;
          setPixelAA(xm + x, ym - y - 1, backColor, i);
          xm = px2;
      ...
      if (e2 + x2 <= 0) {
        ...
        if (i < 256 && i > 0) {
          xm = px2;
          ym = py2;
          setPixelAA(xm - x2 - 1, ym + y, backColor, i);
          xm = px1;
          ym = py2;
          setPixelAA(xm - y, ym - x2 - 1, backColor, i);
          xm = px1;
          ym = py1;
          setPixelAA(xm + x2 + 1, ym - y, backColor, i);
          xm = px2;
  ...
  /**
   * Draws a rectangle with hatched corners, using the current foreground color as the outline color. If both top and
   * bottom are false, nothing is drawn.
   * 
   * @param x left coordinate of the rectangle
   * @param y top coordinate of the rectangle
   * @param width width of the rectangle. the rectangle is drawn from x to x+w-1.
  ...
  /**
   * Fills a rectangle with hatched corners, using the current background color as the fill color. If both top and
   * bottom are false, nothing is drawn.
   * 
   * @param x left coordinate of the rectangle
   * @param y top coordinate of the rectangle
  ...
  private void fillPolygon(int[] xPoints1, int[] yPoints1, int base1, int nPoints1, int[] xPoints2, int[] yPoints2,
      int nPoints2, int c1, int c2, boolean gradient) {
  ...
  private void drawPolygon(int[] xPoints1, int[] yPoints1, int base1, int nPoints1, int[] xPoints2, int[] yPoints2,
      int nPoints2, int c) {
    int i;
  ...
  private void drawLineAA(int x1, int y1, int x2, int y2, int color) {
    ...
    if (surelyOutsideClip(x1, y1, x2, y2)) {
      return;
    }
    ...
    if (DeltaX == 0 && DeltaY == 0) {
      ;
    } else if (DeltaY == 0 || DeltaX == 0 || DeltaX == DeltaY) {
  ...
  private void arcPiePointDrawAndFill(int xc, int yc, int rx, int ry, double startAngle, double endAngle, int c, int c2,
    ...
    if (startIndex > endIndex) {
      ...
      if (fill) {
        fillPolygon(xPoints, yPoints, startIndex, endIndex - startIndex, null, null, 0, gradient ? c : c2, c2,
            gradient);
      }
      if (!gradient) {
        drawPolygon(xPoints, yPoints, startIndex, endIndex - startIndex, null, null, 0, c);
      }
    }
  ...
  private static int interpolate(int color1r, int color1g, int color1b, int color2r, int color2g, int color2b,
      int factor) {
    int m = 255 - factor;
    int r = (color1r * factor + color2r * m) / 255;
    int g = (color1g * factor + color2g * m) / 255;
  ...
  public void copyImageRect(totalcross.ui.image.Image image, int x, int y, int width, int height, int drawOp,
      int backColor, boolean doClip) {
  ...
  public void plotLineAA(int x0, int y0, int x1, int y1) {
    /* draw a black (0) anti-aliased line on white (255) background */
    int sx = x0 < x1 ? 1 : -1, sy = y0 < y1 ? 1 : -1, x2;
    int dx = Math.abs(x1 - x0), dy = Math.abs(y1 - y0), err = dx * dx + dy * dy;
    int e2 = err == 0 ? 1 : (int) (0xFFFF7F / Math.sqrt(err)); /* multiplication factor */
  ...
  @ReplacedByNativeOnDeploy
  public void drawCircleAA(int xm, int ym, int r, boolean fill, boolean tl, boolean tr, boolean bl, boolean br) {
    int x = -r, y = 0;
    ...
    do {
      ...
      if (br) {
        setPixelAA(xm - x, ym + y, foreColor, i);
      }
      if (bl) {
        setPixelAA(xm - y, ym - x, foreColor, i);
      }
      if (tl) {
        setPixelAA(xm + x, ym - y, foreColor, i);
      }
      ...
      if (err + y > 0) {
        ...
        if (i < 256) {
          if (br) {
            setPixelAA(xm - x, ym + y + 1, foreColor, i);
          }
          if (bl) {
            setPixelAA(xm - y - 1, ym - x, foreColor, i);
          }
          if (tl) {
            setPixelAA(xm + x, ym - y - 1, foreColor, i);
          }
      ...
      if (e2 + x2 <= 0) {
        ...
        if (i < 256) {
          if (br) {
            setPixelAA(xm - x2 - 1, ym + y, foreColor, i);
          }
          if (bl) {
            setPixelAA(xm - y, ym - x2 - 1, foreColor, i);
          }
          if (tl) {
            setPixelAA(xm + x2 + 1, ym - y, foreColor, i);
          }
