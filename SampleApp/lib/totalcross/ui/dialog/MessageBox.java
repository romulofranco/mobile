...
public class MessageBox extends Window {
  ...
  public void onPopup() {
    ...
    if (buttonCaptions == null) {
      ...
      {
        multiRow = true;
        btns = new PushButtonGroup(buttonCaptions, false, -1, gap, insideGap, captionCount, true,
            PushButtonGroup.BUTTON);
        btns.setFont(font);
        wb = btns.getPreferredWidth();
      }
    ...
    if (btns != null) {
      ...
      } else if (uiAndroid && !multiRow) {
        btns.setRect(buttonCaptions.length > 1 ? LEFT + 3 : CENTER, ly + hm + androidGap,
            buttonCaptions.length > 1 ? FILL - 3 : Math.max(w / 3, wb), FILL - 2);
      } else {
    ...
    if (btns != null) {
      btns.setBackForeColors(UIColors.messageboxAction,
          Color.getBetterContrast(UIColors.messageboxAction, foreColor, backColor)); // guich@tc123_53
