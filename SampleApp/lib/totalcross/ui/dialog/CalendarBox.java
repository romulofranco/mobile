...
import totalcross.ui.ComboBoxDropDown;
	...
	private void setupUI() // guich@tc100b5_28
	{
		if(!showYearCalendar){
			...
			if(Settings.screenHeight > Settings.screenWidth  )
			{
				...
				lDay.setFont(Font.getFont(true, Font.NORMAL_SIZE + 30));
			...
			calendar = new Container();
			calendarTitle = new Container();
			lowerCalendar = new ScrollContainer(false, true);
			calendarTitle.setForeColor(fore);
			titleYear = new Label();
			titleYear.setText(year + "");
			...
			if(Settings.screenHeight > Settings.screenWidth  )
			{
				...
				calendar.add(lowerCalendar, LEFT, AFTER, FILL, FIT,calendarTitle);
				...
				int aux = years.size();
			    for(int i = 0; i++ < aux;) {
					lowerCalendar.add(years.get(i - 1), CENTER, i == 0? TOP: AFTER, PARENTSIZE, DP + 57);
				...
				add(calendar, LEFT, TOP, FILL, FILL);
				calendar.add(calendarTitle, LEFT, TOP, SCREENSIZE + (1680/72), FILL);
				calendar.add(lowerCalendar, AFTER, TOP, FILL, Settings.screenHeight*246/360);
				...
				int aux = years.size();
				for(int i = 0; i++ < aux;) {
					lowerCalendar.add(years.get(i - 1), CENTER, i == 0? TOP: AFTER, PARENTSIZE, DP + 48);
