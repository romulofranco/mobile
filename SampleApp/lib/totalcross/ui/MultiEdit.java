...
public class MultiEdit extends Container implements Scrollable, TextControl, HasValue<String> {
  ...
  protected void onBoundsChanged(boolean screenChanged) {
    ...
    int caph = uiMaterial && caption != null ? materialCaption.getExtraHeight() : 0;
  ...
  public void onEvent(Event event) {
    ...
    if (event.target == this && textRect != null) {
      ...
      case TimerEvent.TRIGGERED:
        ...
        if (editable && isEnabled() && lastPenDown != -1 && Edit.clipboardDelay != -1
            && (Vm.getTimeStamp() - lastPenDown) >= Edit.clipboardDelay) {
  ...
  private void showSip() // guich@tc126_21
  {
        ...
        && !Window.isScreenShifted() && Settings.enableVirtualKeyboard) // if running on a PocketPC device, set the bounds of Sip in a way to not cover the edit - kmeehl@tc100: added check for editMode and !dragScroll
    {
      if (tea != null) {
        postPopupKCC = true;
      } else if (Settings.customKeyboard != null) {
    ...
    if (!transparentBackground) {
      if (uiMaterial) {
        ...
        if (uiAndroid) {
          if (npback == null) {
            try {
              npback = NinePatch.getInstance().getNormalInstance(NinePatch.MULTIEDIT, width, height, isEnabled() ? back0
